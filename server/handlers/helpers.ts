...
export const query: Handler = (req, res, next) => {
  ...
  if (
    ...
    return res.status(400).json({ error: "A limit lekérése nem érvényes" });
  }
  ...
  if (
    ...
    return res.status(400).json({ error: "A kihagyás kérése nem érvényes" });
  }
  ...
  if (
    ...
    return res.status(400).json({ error: "A keresési kérés nem érvényes" });
  }
